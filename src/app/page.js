        ...
        <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
          ...
          {open && suggestions.length > 0 && (
            <div style={{
              ...
              {suggestions.map(c => (
                <a key={c.id} href={`/pathiseis/${encodeURIComponent(c.slug)}${lang === 'en' ? '?lang=en' : ''}`}
                  style={{
...
export default function HomePage() {
  ...
  function onKey(e) {
    ...
    if (suggestions.length > 0) {
      window.location.href = `/pathiseis/${encodeURIComponent(suggestions[0].slug)}${lang === 'en' ? '?lang=en' : ''}`;
      return;
    }
      ...
      <Section bg={C.off}>
        ...
        <div className="pv-chips">
          {(conditions.length ? conditions : FALLBACK_CHIPS[lang].map((n, i) => ({ id: i, slug: slugify(n), name_el: n, name_en: n }))).map(c => {
            ...
            return (
              <a key={c.id} href={`/pathiseis/${c.slug || slugify(label)}${lang === 'en' ? '?lang=en' : ''}`}
