...
export default function TherapistDashboard() {
  ...
  async function respondAppointment(id, status) {
    ...
    if (status === 'accepted') {
      ...
      if (appt?.slot_id) {
        await supabase.from('availability_slots').update({ is_blocked: true }).eq('id', appt.slot_id);
      }
    }
  ...
  async function toggleSlot(day, hour) {
    ...
    if (existing) {
      ...
      const { data } = await supabase.from('availability_slots').insert([{
        therapist_id: user.id, date: day, start_time: hour + ':00', end_time: (parseInt(hour) + 1) + ':00:00', is_blocked: false,
      }]).select().single();
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {!profile?.is_approved && (
            <span style={{ background: '#FEF3C7', color: '#92400E', padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600 }}>⏳ Εκκρεμεί έγκριση admin</span>
          )}
        ...
        {activeTab === 'overview' && (
          <div>
            <div style={{ display: 'flex', gap: 14, marginBottom: 24, flexWrap: 'wrap' }}>
              {[
                { label: 'Εκκρεμή Αιτήματα', value: pending, bg: '#FEF3C7', border: '#FDE68A', text: '#B45309' },
                ...
                { label: 'Έσοδα (εκτίμηση)', value: `${earned}€`, bg: '#F0FDF4', border: '#BBF7D0', text: '#15803D' },
              ].map(c => (
                ...
                <div key={a.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', padding: '18px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {a.service && (
                        <div style={{ fontSize: 13, color: '#475569', background: '#f8fafc', padding: '8px 12px', borderRadius: 8, borderLeft: '3px solid #cbd5e1', marginTop: 6 }}>
                          {a.service}{a.notes ? ` — ${a.notes}` : ''}
                        </div>
                      )}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                      {a.status === 'pending' && (
                        <>
                          <button onClick={() => respondAppointment(a.id, 'accepted')}
                            style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#15803D', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                            ✓ Αποδοχή
                          </button>
                          <button onClick={() => respondAppointment(a.id, 'rejected')}
                            style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #FECDD3', background: 'transparent', color: '#BE123C', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                            ✕ Απόρριψη
                          </button>
                        </>
                      )}
                      {a.status === 'accepted' && (
                        <button onClick={() => markCompleted(a.id)}
                          style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#7C3AED', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                          ✓ Ολοκλήρωση
                        </button>
                      )}
                    </div>
        ...
        {activeTab === 'calendar' && (
          <div>
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', padding: 24, marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', marginBottom: 6 }}>📅 Εβδομαδιαία Διαθεσιμότητα</div>
              <div style={{ fontSize: 13, color: '#64748B', marginBottom: 20 }}>Κλικάρετε στα κελιά για να ορίσετε τις διαθέσιμες ώρες σας.</div>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 600 }}>
                  <thead>
                    <tr>
                      <th style={{ padding: '8px 12px', fontSize: 12, color: '#64748B', fontWeight: 600, textAlign: 'left' }}>Ώρα</th>
                      {nextWeek.map((d, i) => (
                        <th key={d} style={{ padding: '8px 12px', fontSize: 12, color: '#64748B', fontWeight: 600, textAlign: 'center' }}>
                          {DAYS[i]}<br /><span style={{ fontSize: 11, fontWeight: 400 }}>{new Date(d).toLocaleDateString('el-GR', { day: '2-digit', month: '2-digit' })}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {HOURS.map(hour => (
                      <tr key={hour}>
                        <td style={{ padding: '6px 12px', fontSize: 12, color: '#475569', fontWeight: 600 }}>{hour}</td>
                        {nextWeek.map(day => {
                          const slot = slots.find(s => s.date === day && s.start_time === hour + ':00');
                          const isBlocked = slot?.is_blocked;
                          const isAvailable = slot && !isBlocked;
                          return (
                            <td key={day} style={{ padding: 4, textAlign: 'center' }}>
                              <div onClick={() => !isBlocked && toggleSlot(day, hour)}
                                style={{ width: '100%', height: 32, borderRadius: 6, cursor: isBlocked ? 'not-allowed' : 'pointer', background: isBlocked ? '#FEE2E2' : isAvailable ? '#D1FAE5' : '#F1F5F9', border: `1px solid ${isBlocked ? '#FECACA' : isAvailable ? '#BBF7D0' : '#E2E8F0'}`, transition: 'all .15s', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 }}>
        ...
        {activeTab === 'reviews' && (
          <div>
            <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 140, background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 14, padding: '18px 20px' }}>
              ...
              <div style={{ flex: 1, minWidth: 140, background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 14, padding: '18px 20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
              ...
              <div>
                ...
                {profile?.is_approved ? (
                  ...
                  <Badge label="⏳ Αναμένει έγκριση" bg="#FEF3C7" color="#92400E" />
                )}
              </div>
              <button onClick={() => setEditProfile(!editProfile)} style={{ marginLeft: 'auto', padding: '8px 18px', borderRadius: 8, border: '1px solid #e2e8f0', background: editProfile ? '#f1f5f9' : '#fff', color: '#475569', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            ...
            {editProfile ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  ['Καθαρά ανά συνεδρία', profile?.price_per_session ? `${profile.price_per_session - commission}€` : '—'],
                ].map(([label, value]) => (
