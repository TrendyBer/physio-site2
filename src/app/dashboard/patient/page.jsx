...
// Helper: friendly date label (ΣΗΜΕΡΑ / ΑΥΡΙΟ / ΣΕ X ΜΕΡΕΣ)
...
export default function PatientDashboard() {
  ...
  const [appointmentsView, setAppointmentsView] = useState('list'); // 'list' | 'calendar'
  ...
  // ═══ FLATTEN bookings into single list with request info attached ═══
  ...
  const upcomingAppointments = sortedAppointments.filter(a => {
    ...
    return d >= now || a.payment_status === 'held'; // also include held even if past time
  });
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}>
      <div style={{ fontSize: 18, color: '#64748B' }}>Φόρτωση...</div>
    </div>
  );

  const TABS = [
    { id: 'appointments', label: 'Τα Ραντεβού μου', Icon: Calendar },
  ...
  // ═══ CALENDAR HELPER: Build month grid ═══
  ...
  // Map appointments by date for calendar lookup
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#0F172A' }}>Καλώς ήρθατε, {profile?.name?.split(' ')[0] || 'Ασθενής'}</h1>
          <p style={{ fontSize: 15, color: '#64748B', marginTop: 4 }}>Διαχειριστείτε τα ραντεβού και τα αιτήματά σας.</p>
        </div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #FEF3C7, #FDE68A)', border: '1px solid #F59E0B', borderRadius: 14, padding: '18px 22px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <div style={{ fontSize: 14, color: '#78350F', lineHeight: 1.5 }}>
                Ο θεραπευτής δηλώνει ότι ολοκληρώθηκαν. Επιβεβαιώστε για να απελευθερωθεί η πληρωμή ({heldAmount.toFixed(2)}€ συνολικά).
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ background: '#1a2e44', borderRadius: 14, padding: '20px 24px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700, color: '#fff', marginBottom: 4 }}>Θέλετε νέο ραντεβού;</div>
            <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.7)' }}>Κλείστε ραντεβού με έναν από τους θεραπευτές μας</div>
          </div>
                  ...
                  <div style={{ marginBottom: 28 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {upcomingAppointments.map(apt => {
                        const bSt = BOOKING_STATUS[apt.status] || BOOKING_STATUS.pending;
                            ...
                            {isHeld && (
                              <div style={{ marginTop: 14, padding: 14, background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 10 }}>
                                ...
                                <button onClick={() => openReleaseModal(apt, apt.request)}
                                  ...
                                  Απελευθέρωση {parseFloat(apt.session_amount || 0).toFixed(2)}€
                                </button>
                              </div>
                            )}
                          </div>
                ...
                <div key={req.id} style={{ background: '#fff', borderRadius: 14, border: reqHeldBookings.length > 0 ? '2px solid #F59E0B' : '1px solid #e2e8f0', overflow: 'hidden' }}>
                  <div style={{ padding: '18px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                      <span style={{ fontWeight: 700, fontSize: 16, color: '#0F172A' }}>{req.problem_type || 'Φυσιοθεραπεία'}</span>
                      ...
                      <span style={{ fontSize: 12, color: '#94A3B8', marginLeft: 'auto' }}>{new Date(req.created_at).toLocaleDateString('el-GR')}</span>
                    </div>
                  ...
                  {req.review ? (
                    ...
                    <div style={{ padding: '14px 20px', borderTop: '1px solid #f1f5f9', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => openReviewModal(req)}
                        style={{ padding: '10px 20px', borderRadius: 30, border: 'none', background: '#F59E0B', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        ...
        {activeTab === 'services' && (
          <div>
            <div style={{ marginBottom: 20 }}>
              <h2 style={{ fontSize: 19, fontWeight: 700, color: '#0F172A', marginBottom: 4 }}>Υπηρεσίες Φυσιοθεραπείας</h2>
              <p style={{ fontSize: 14, color: '#64748B' }}>Εξατομικευμένη φροντίδα για ένα εύρος παθήσεων.</p>
            </div>
                ...
                {services.map(s => (
                  <div key={s.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', padding: 20, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                      ...
                      <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0F172A', lineHeight: 1.3 }}>{s.title_el}</h3>
                    </div>
                    {s.desc_el && (
                      <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.6, marginBottom: 16, flex: 1 }}>{s.desc_el}</p>
                    )}
        ...
        {activeTab === 'profile' && (
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #e2e8f0', padding: 28 }}>
            ...
            <h3 style={{ fontSize: 17, fontWeight: 700, color: '#0F172A', marginBottom: 4 }}>Στοιχεία Προφίλ</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>Ονοματεπώνυμο *</label>
                <input
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>Τηλέφωνο</label>
                  <input
                ...
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>Περιοχή</label>
                  <input
              ...
              <div>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>Διεύθυνση</label>
                <input
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>Πόλη</label>
                  <input
                ...
                <div>
                  <label style={{ fontSize: 13, fontWeight: 600, color: '#1a2e44', display: 'block', marginBottom: 5 }}>ΤΚ</label>
                  <input
      ...
      {releaseModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: '32px', maxWidth: 480, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <h2 style={{ fontSize: 22, fontWeight: 700, color: '#0F172A', marginBottom: 12, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setReleaseModal(null)} disabled={releasing}
                style={{ flex: 1, padding: '14px', borderRadius: 30, border: '1px solid #e2e8f0', background: 'transparent', color: '#64748b', fontSize: 15, fontWeight: 600, cursor: releasing ? 'not-allowed' : 'pointer' }}>
                Ακύρωση
              </button>
              <button onClick={confirmRelease} disabled={releasing}
                style={{ flex: 2, padding: '14px', borderRadius: 30, border: 'none', background: releasing ? '#94a3b8' : '#15803D', color: '#fff', fontSize: 15, fontWeight: 600, cursor: releasing ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <CheckCircle2 size={16} strokeWidth={2.5} />
      ...
      {reviewModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}
          ...
          <div style={{ background: '#fff', borderRadius: 20, padding: 32, maxWidth: 460, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ fontSize: 21, fontWeight: 700, color: '#0F172A', marginBottom: 8 }}>Αξιολόγηση Θεραπευτή</h2>
            ...
            <div style={{ marginBottom: 20, padding: '16px 20px', background: '#FFFBEB', borderRadius: 12, border: '1px solid #FDE68A' }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#92400E', marginBottom: 10 }}>Βαθμολογία *</div>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ fontSize: 14, fontWeight: 600, color: '#0F172A', display: 'block', marginBottom: 8 }}>Σχόλιο (προαιρετικό)</label>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setReviewModal(null)} disabled={submittingReview}
                style={{ flex: 1, padding: '13px', borderRadius: 30, border: '1px solid #e2e8f0', background: 'transparent', color: '#64748b', fontSize: 15, fontWeight: 600, cursor: submittingReview ? 'not-allowed' : 'pointer' }}>
                Άκυρο
              </button>
              <button onClick={submitReview} disabled={!reviewForm.rating || submittingReview}
                style={{ flex: 2, padding: '13px', borderRadius: 30, border: 'none', background: reviewForm.rating ? '#F59E0B' : '#e2e8f0', color: reviewForm.rating ? '#fff' : '#94a3b8', fontSize: 15, fontWeight: 600, cursor: reviewForm.rating ? 'pointer' : 'not-allowed', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                <Star size={15} fill={reviewForm.rating ? '#fff' : '#94a3b8'} strokeWidth={0} />
                {submittingReview ? 'Αποστολή...' : 'Υποβολή'}
              </button>
            </div>
          </div>
        </div>
